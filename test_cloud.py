...
class Connecter(communication_pb2_grpc.CommServicer):
    ...
    def process_device_output(self, request, context):
        ...
        print("device output start time: ", self.device_return_time)
    ...
    def process_cloud_output(self, request, context):
        t = time.time()
        print('process cloud output start time:', t)
        ...
        self.edge_return_time = time.time()
        if self.device_return_time != 0:
            print("device return ahead edge time: ", (self.device_return_time - self.edge_return_time) * 1000.)
    ...
    def get_one_layer_gradients(self, request, context):
        ...
        if name == 'cloud':
            self.edge_come_time = time.time()
        if name == 'device':
            t1 = time.time()
            print("edge wait device time: ", (t1 - self.edge_come_time)*1000)
    ...
    def device_equal_cloud(self, out, label):
        ...
        tmp_ts2 = time.time()
        print("get output done cost time: ", (tmp_ts2 - tmp_ts1) * 1000.)
    ...
    def process_raw_data(self, request, context):
        ...
        print("receive raw data time, ", self.tmp_ts1)
