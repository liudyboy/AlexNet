...
class Connecter(communication_pb2_grpc.CommServicer):
    ...
    def process_cloud_output(self, request, context):
        if self.init_layers_flag is False:
            ...
            paritition_point = 2
            self.use_gpu = True
            self.lenet = LeNet5(self.use_gpu)
        ...
        output = self.lenet.forward(self.edge_output_x, self.process_layers)
        output_reply = self.lenet.cal_gradients(output, self.process_layers, self.edge_output_y)
        self.lenet.update_layers_parameters(self.process_layers)
        grads = output_reply
        grads.to_cpu()
