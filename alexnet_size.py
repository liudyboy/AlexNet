...
class AlexNet():
    ...
    def forward(self, x):
        ...
        np.savez("layer1_forward", x=out.array)
        out = self.max_pool1.forward(out)
        np.savez("layer2_forward", x=out.array)

        out = self.conv2.forward(out)
        np.savez("layer3_forward", x=out.array)
        out = self.max_pool2.forward(out)
        np.savez("layer4_forward", x=out.array)



        out = self.conv3.forward(out)

        np.savez("layer5_forward", x=out.array)

        out = self.conv4.forward(out)

        np.savez("layer6_forward", x=out.array)
        out = self.conv5.forward(out)

        np.savez("layer7_forward", x=out.array)
        out = self.max_pool5.forward(out)

        np.savez("layer8_forward", x=out.array)
        out = self.fc6.forward(out)

        np.savez("layer9_forward", x=out.array)
        out = self.fc7.forward(out)

        np.savez("layer10_forward", x=out.array)
        out = self.fc8.forward(out)

        np.savez("layer11_forward", x=out.array)
    ...
    def backward(self, loss, temp_out):
        ...
        np.savez("layer10_gradeints", x=d_out.array)
        d_out = self.fc7.backward(d_out)
        np.savez("layer9_gradeints", x=d_out.array)
        d_out = self.fc6.backward(d_out)

        np.savez("layer8_gradeints", x=d_out.array)
        d_out = self.max_pool5.backward(d_out)
        np.savez("layer7_gradeints", x=d_out.array)
        d_out = self.conv5.backward(d_out)

        np.savez("layer6_gradeints", x=d_out.array)
        d_out = self.conv4.backward(d_out)
        np.savez("layer5_gradeints", x=d_out.array)
        d_out = self.conv3.backward(d_out)

        np.savez("layer4_gradeints", x=d_out.array)
        d_out = self.max_pool2.backward(d_out)
        np.savez("layer3_gradeints", x=d_out.array)
        d_out = self.conv2.backward(d_out)

        np.savez("layer2_gradeints", x=d_out.array)
        d_out = self.max_pool1.backward(d_out)
        np.savez("layer1_gradeints", x=d_out.array)
