...
if __name__ == "__main__":
    ...
    for i in range(generations):
        ...
        np.savez('cloud', x=cloud_X.array, y=cloud_Y.array)
        np.savez('edge', x=edge_X.array, y=edge_Y.array)
        try:
            if cloud_run_layers != 0:
                _thread.start_new_thread(send_raw_data, ("send edge thread", 0, cloud_address, cloud_X, cloud_Y))
            print("send data shape: ", edge_X.shape)
            _thread.start_new_thread(send_raw_data, ("send cloud thread", 1, edge_address, edge_X, edge_Y))
        except:
        ...
        if device_run_layers != 0:
            ...
            t = time.time()
            print("send cloud  output data time:", t)
