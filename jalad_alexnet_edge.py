...
class Connecter(communication_pb2_grpc.CommServicer):
    ...
    def process_raw_data(self, request, context):
        ...
        out = self.alexnet.forward(raw_input, self.process_layers)
        output_reply = self.send_output_data(self.cloud_address, out, self.Y)
        out = self.alexnet.cal_gradients(output_reply, self.process_layers, self.Y)
        self.alexnet.update_layers_parameters(self.process_layers, batch_size)
